...
$master = $_GET['master'];//接收插件传来的主人
$admin = $_GET['admin'];//接收插件传来的管理员，管理员是数组形式，需要自行处理
$at = $_GET['at'];//接收插件传来的得到艾特列表，数组形式
...
//还有msg，代表消息内容，msg就是要发送的内容，ptt就是语音链接，图片也是链接
...
if ($msg == "全禁")//如果消息是测试全禁
{
    echo json(add("sendT","msg").add("msg","成功").add("do","全禁").add("全禁","1108960905 0"));
}
else if ($msg == "全解")
{
    echo json(add("sendT","msg").add("msg","成功").add("do","全禁").add("全禁","1108960905 1"));
}



else//这个什么也不用回复，因为不写这个，不发指令的内容会显示配置不正常
{
    echo json(add("sendT","").add("msg","").add("do","null"));
}
